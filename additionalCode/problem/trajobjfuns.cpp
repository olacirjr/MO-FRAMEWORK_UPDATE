...
double sagas(const vector<double>& x, double& DVtot, double& DVonboard){
	mgadsmproblem problem;
...
double tandem_wbounds(double* aa, double& tof, const int sequence_[]){
	mgadsmproblem problem;
        double auxval;

        std::vector<double> x;
	for (int i = 0; i < decisionNumber; i++){
		auxval = MinTandemBounds[i]+aa[i]*(MaxTandemBounds[i]-MinTandemBounds[i]); 
		x.push_back(auxval);             
	} 
	...
	sumDVvec=sumDVvec+0.165; //losses for 3 swgbys + insertion
        //printf("A %f %f %f %f %f %f \n ",m_initial,VINFE,declination,sumDVvec,Isp,g0);
        //printf("B %f %f %f %f \n ",sumDVvec/Isp,(sumDVvec/Isp)/g0,-((sumDVvec/Isp)/g0)*1000,exp(-((sumDVvec/Isp)/g0)*1000));
