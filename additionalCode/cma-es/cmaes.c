...
// static double std::max( double d1, double d2);
// static double std::min( double d1, double d2);
...
// // //   }
  ...
  */
  ...
  if(psxps/N > 1.5 + 10.*sqrt(2./N) 
    ...
    double tfac = sqrt((1 + std::max(0, log(psxps/N))) * N / psxps);
    for (i=0; i<N; ++i) 
      t->rgps[i] *= tfac;
    psxps *= tfac*tfac; 
  }
  ...
  printf("\nevo ccov: %.20f", t->sp.ccov);
  printf("\nevo mucov: %.20f", t->sp.mucov);
  printf("\nevo mueff: %.20f", t->sp.mueff);
  printf("\nevo BDz: ");
  ...
  printf("\nevo psxps: %f", psxps);
  printf("\nevo chin: %f", t->chiN);
  printf("\nevo cSigma: %f", t->sp.cs);
  printf("\nevo dSigma: %f", t->sp.damps);
  printf("\nevo sigma: %f", t->sigma);
...
Adapt_C2(cmaes_t *t, int hsig)
{
  ...
  if (t->sp.ccov != 0. && t->flgIniphase == 0) {
    ...
    printf("\n\nevo ccov1 %f", ccov1);
    printf("\nevo ccovmu %f", ccovmu);
    printf("\nevo sigmasquare %.20f", sigmasquare);
    printf("\nevo hsig %d", hsig);
    printf("\nevo ccumcov %f", t->sp.ccumcov);
    printf("\nevo fldiag %d", flgdiag);
...
cmaes_TestForTermination( cmaes_t *t)
{
  ...
  cp[0] = '\0';
      ...
      /* TolFun */
      range = std::max(rgdouMax(t->arFuncValueHist, (int)std::min(t->gen,*(t->arFuncValueHist-1))), 
...
*/
{
  ...
  t->damps = t->damps 
    * (1 + 2*std::max(0., sqrt((t->mueff-1.)/(N+1.)) - 1))     /* basic factor */
    * std::max(0.3, 1. -                                       /* modify for short runs */
                  (double)N / (1e-6+std::min(t->stopMaxIter, t->stopMaxFunEvals/t->lambda))) 
...
// static double
// std::max( double i, double j)
// {
//   return i > j ? i : j;
// }
// static double
// std::min( double i, double j)
// {
//   return i < j ? i : j;
// }
static double
